...
import { differenceInMonths } from "npm:date-fns@2.29.3";
...
const AboutPage = ({ route }) => (
  <Layout route={route}>
    ...
    <Box header="About this site">
      <p>
        {`This site was built using ${data["built using"]} and is running on ${
          data["running on"]
        }`}
      </p>
    ...
    <Box header="Experience">
      ...
      <p>
        ...
        <A href="https://charanga.com">{`Charanga`}</A>
        {`, started in 2015 (${diff = differenceInMonths(
          Date.now(),
          new Date(2015, 1, 23),
        ),
          Math.floor(diff / 12)}${
          diff % 12 > 6 ? "½" : ""
        } years), building Android, iOS and Desktop apps primarily in JavaScript. Initially using Cordova and AngularJS, but these days using Electron/React and React Native`}
      </p>
